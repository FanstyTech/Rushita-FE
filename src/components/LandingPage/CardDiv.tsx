...
function CardDiv({ Icon, title, doctorname, rotate, className, bgcolor }: { bgcolor: string, className?: string, rotate: number, Icon: LucideIcon, title: string, doctorname: string }) {
    return (
        <div style={{ rotate: `${rotate}deg` }} className={`w-full  h-full absolute -top-10 ltr:-left-10 -right-10  rounded-full pointer-events-none ${className}`}>
            <div style={{ rotate: `${rotate * -1}deg` }} className="max-w-[210px] px-3 py-3 xl:scale-100  scale-70 bg-card shadow-md rounded-lg">
                <div className="flex ">
                ...
                <p className="text-foreground/70 text-left" style={{ fontSize: "10px" }}  >{doctorname}</p>
            </div>
        </div>)
}
