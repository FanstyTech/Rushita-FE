...
import { DoctorDashboardChartDataDto, TimeRange } from '@/lib/api/types/doctor-dashboard';
...
export function DashboardChart({ chartData, timeRange }: DashboardChartProps) {
  ...
  switch (timeRange) {
    case TimeRange.Daily:
      data = chartData.daily?.labels.map((label, index) => ({
        time: label,
    ...
    case TimeRange.Weekly:
      data = chartData.weekly?.labels.map((label, index) => ({
        time: label,
    ...
    case TimeRange.Monthly:
      data = chartData.monthly?.labels.map((label, index) => ({
        time: label,
