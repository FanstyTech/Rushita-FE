...
const mockData = {
  ...
  chartData: {
    ...
    yearly: {
      ...
      appointments: [820, 780, 920, 980, 1100, 1050, 1080, 990, 1040, 980, 1060, 1100]
    }
  },
  activities: [
    { id: 1, type: 'clinic_added', clinic: 'City Medical Center', time: '2 hours ago' },
    { id: 2, type: 'user_joined', name: 'Dr. Sarah Wilson', role: 'Doctor', time: '4 hours ago' },
    { id: 3, type: 'appointment_surge', clinic: 'Metro Hospital', count: 25, time: '6 hours ago' },
...
interface ActivityItem {
  id: number;
  type: 'clinic_added' | 'user_joined' | 'appointment_surge' | 'system_update';
  clinic?: string;
  name?: string;
  role?: string;
  count?: number;
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`mt-2 text-sm inline-flex items-center rounded-full px-2 py-1 ${
                      stat.changeType === 'increase' ? 'bg-green-400/20 text-green-100' : 'bg-red-400/20 text-red-100'
                    }`}>
                      {stat.change} from last month
                    </p>
                  </div>
                  <div className="rounded-xl bg-white/10 p-3 backdrop-blur-sm">{stat.icon}</div>
                </div>
              </div>
              ...
              {activities.map((activity) => (
                <div key={activity.id} className="flex items-start gap-4 group">
                  <div className={`rounded-xl p-2 transition-all duration-200 ${
                    activity.type === 'clinic_added' ? 'bg-blue-100 text-blue-600 group-hover:bg-blue-200' :
                    activity.type === 'user_joined' ? 'bg-emerald-100 text-emerald-600 group-hover:bg-emerald-200' :
                    activity.type === 'appointment_surge' ? 'bg-amber-100 text-amber-600 group-hover:bg-amber-200' :
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900 group-hover:text-gray-700 transition-colors duration-200">
                      {activity.type === 'clinic_added' && `New clinic added: ${activity.clinic}`}
                      {activity.type === 'user_joined' && `${activity.name} joined as ${activity.role}`}
                      {activity.type === 'appointment_surge' && `High booking activity at ${activity.clinic}`}
                      {activity.type === 'system_update' && `System updated to version ${activity.version}`}
                    </p>
