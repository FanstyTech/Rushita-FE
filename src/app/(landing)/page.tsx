...
export default function Home() {
  ...
  if (loading) {
    return (
      <Card className="w-screen fixed top-0 left-0  z-50 h-screen flex justify-center items-center">
    ...
    >
          ...
          <div className="container mx-auto px-6 py-20">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {whyroshita.map((item, index) => (
                <div
                  key={index}
                  className="group relative backdrop-blur-md bg-white/10 dark:bg-black/10 rounded-3xl p-1 hover:bg-white/20 dark:hover:bg-black/20 transition-all duration-500 hover:scale-[1.02]"
                  style={{
                    background:
                ...
                >
                  ...
                  <div className="relative overflow-hidden rounded-3xl">
                    ...
                    <div className="relative p-6 backdrop-blur-sm bg-white/20 dark:bg-black/20">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                        {item.Title}
                      </h3>

                      {/* Animated line */}
                      <div className="h-px bg-gradient-to-r from-blue-400 to-purple-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
                    </div>
                  </div>

                  {/* Glow effect */}
                  <div className="absolute -inset-px bg-gradient-to-r from-blue-500/20 to-purple-500/20 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10 blur-sm" />
                </div>
              ))}
            </div>
          </div>
      ...
      <Card className="md:pb-20 pb-10 rounded-none border-0">
        <h1 className="xxs:text-3xl text-xl max-w-6xl relative mx-auto leading-tight font-normal text-center sm:text-5xl  p-4 bg-gradient-to-tr to-secend from-primary bg-clip-text text-transparent">
          روشيتة خلصتك من كل هالفوضى، بنظام يشوف عنك، يتذكّر عنك، ويرتب لك كل
          شيء من أول لحظة.
        </h1>
      </Card>
      ...
      <div
        id="whyroshita"
        className="py-20 bg-gray-bg relative overflow-hidden"
      >
      ...
      <Card className=" p-0 pt-7 border-0 rounded-none">
        ...
        <div className="md:mt-10 mt-5 max-w-7xl w-full px-4 sm:px-6 lg:px-8 mx-auto md:min-h-[60vh] ">
          <div className="grid  sm:grid-cols-2 grid-cols-1 gap-5 w-full h-full">
            <div className="flex items-center ltr:ml-auto">
              <div className="">
                ...
                >
                  ...
                  <p className="text-foreground/70  md:text-sm text-xs font-semibold">
                    {`"روشيتة"`} انولد من قلب عيادة.. اشتغلنا مع أطباء،
                    سكرتارية، مرضى سمعنا كل التفاصيل، كل المآسي، وكل آه صغيرة من
                    ضغط يوم العيادة.
                  </p>
                  <p className="text-secend md:text-sm text-xs font-semibold">
                    ورجعنا صممنا النظام مش عشان يكون {`"جميل"`}وبس! ، لكن عشان
                    يكون مفيد – سريع – واقعي – ذكي.
                  </p>
                </div>
            ...
            <div
              className="relative rounded-4xl w-full sm:h-full h-[50vh] bg-center bg-cover "
              style={{
      ...
      <div
        id="projects"
        className="relative bg-gradient-to-t to-transparent from-gray-50 overflow-hidden"
      >
        ...
        <Card className="md:pt-10 pt-5 border-0 rounded-none bg-gray-100 dark:bg-gray-700    px-4 sm:px-6 lg:px-8 sm:min-h-[60vh] ">
          <div className="flex md:flex-nowrap flex-col-reverse md:flex-row flex-wrap justify-between w-full h-full">
            <div className="md:w-1/2 w-full">
              <div
                className="relative rounded-4xl lg:w-4/5 w-full md:h-full h-[50vh] bg-center bg-contain bg-no-repeat "
                style={{ backgroundImage: "url('/images/image349.png')" }}
              >
                <div
                  className=" rounded-4xl xxs:block hidden w-[218px]  absolute sm:bottom-16 bottom-10 ltr:right-0 left-0  sm:left-10  lg:ltr:left-0 md:-left-10   lg:-left-20  h-[100px] bg-center bg-cover bg-no-repeat "
                    ...
                    <ul className="flex-col flex gap-3 text-foreground/70 sm:text-sm text-xs">
                      {whatswepost.map((item, index) => (
                        <li key={index} className="flex gap-2 ">
                          {' '}
                          <div className="min-h-2.5 mt-2 ltr:mt-1 min-w-2.5 max-h-2.5 max-w-2.5   bg-secend rotate-45"></div>{' '}
                          {item}
                        </li>
                      ))}
                    </ul>
              ...
              <div className="max-w-[540px] flex flex-col gap-5">
                <div className="lg:text-5xl md:text-4xl sm:text-3xl xxs:text-2xl text-xl font-normal md:leading-16">
                  <h1 className="">
                    ...
                    <span>بيشتغل كأنه فريق كامل</span>
                  </h1>
                  <h1></h1>
                </div>

                <p className="text-foreground/70 md:text-lg text-sm">
                  من لحظة دخول المريض، لجدولة الموعد، للتشخيص، للوصفة،
                  للمتابعة.. كل قسم في العيادة يتواصل تلقائيًا مع الثاني.
                </p>
                <ul className="flex-col flex gap-3 text-foreground/80 md:text-sm text-xs">
                  {roshtasystem.map((item, index) => (
                    <li key={index} className="flex gap-2 ">
                      {' '}
                      <div className="min-h-2.5 mt-1 min-w-2.5 max-h-2.5 max-w-2.5   bg-secend rotate-45"></div>{' '}
                      {item}
                    </li>
                  ))}
                </ul>
      ...
      <Card
        className="bg-gray-100 dark:bg-gray-700 pt-10  rounded-none border-0"
        id="contact"
      >
        <h1 className="xxs:text-3xl text-xl max-w-3xl   mx-auto leading-tight font-normal text-center sm:text-5xl  p-4 bg-gradient-to-tr to-secend from-primary bg-clip-text text-transparent">
        ...
        <p className="text-foreground/70 max-w-5xl mx-auto sm:text-lg xxs:text-base text-sm  text-center leading-8 w-[98%]">
          روشيتة وُلدت من قلب العيادات، بعد سنوات من الاستماع للأطباء، الموظفين،
          والمرضى. مش بس برنامج… بل شريك رقمي حقيقي، صُمم ليحل التحديات اليومية
          ويُعيد ترتيب يوم العمل الطبي
        </p>
        <SubscriptionSection />
      </Card>
    </div>
